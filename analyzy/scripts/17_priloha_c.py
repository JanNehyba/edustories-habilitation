...
def main() -> None:
    ...
    md.append("")
    md.append("Tato příloha dokládá registr anotačních událostí, multiplicitu anotací,")
    md.append("rekonciliaci velikostí datových řezů a kanonický přehled měr shody.")
    md.append("Úplná matice pokrytí (kazuistika × událost) je dostupná v companion")
    ...
    md.append("")
    md.append("**Tabulka C.2.** Kolikrát byly kazuistiky zasaženy lidskými anotačními událostmi a etapami.")
    md.append("")
    md.append("| Ukazatel | Kazuistik |")
    md.append("|---|---|")
    md.append(f"| právě jedna lidská událost | {a_c('c2_multiplicita_1x', mult[1])} |")
    md.append(f"| dvě události | {a4('multiplicita_2x')} |")
    md.append(f"| tři události | {a4('multiplicita_3x')} |")
    md.append(f"| čtyři události | {a4('multiplicita_4x')} |")
    md.append(f"| alespoň jedna lidská událost | {a4('prekryv_ge1_udalost')} |")
    md.append(f"| více než jedna událost | {a4('prekryv_ge2_udalosti')} |")
    ...
    md.append("")
    md.append("**Tabulka C.3.** Čísla, která se v knize a podkladech vztahují k „velikosti dat“, a jejich vztah.")
    md.append("")
    ...
    md.append("")
    md.append("**Tabulka C.4.** Shrnutí spolehlivosti napříč etapami (podrobnosti a intervaly spolehlivosti v kapitolách 6 a 7).")
    md.append("")
    ...
    md.append("- [x] Tabulky číslované C.1–C.4, s uvedeným zdrojem")
    md.append("- [x] Anonymizace: pouze pseudonymy A1–A12")
    md.append("- [x] Žádný em-dash; en-dash jen v rozsazích")
    md.append("- [ ] Po K3 freeze regenerovat (řádek K3 finál a C.4) skriptem 17")
